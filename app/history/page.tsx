...
const HistoryView = () => {
  ...
  // ★ 設定を localStorage から読む（リロードしても維持）
  ...
  const handleFromChange = (newFrom: string) => { setCustomFrom(newFrom); if (newFrom > customTo) setCustomTo(newFrom); setRangeError(''); };
  const handleToChange = (newTo: string) => { setCustomTo(newTo); if (newTo < customFrom) setCustomFrom(newTo); setRangeError(''); };
  ...
  const fetchData = async () => {
    ...
    const { data: configData } = await supabase
      .from('bot_configs').select('eth_balance, target_roi, user_name').eq('user_id', userId).maybeSingle();
    if (configData) setConfig({ eth_balance: Number(configData.eth_balance), target_roi: Number(configData.target_roi), user_name: configData.user_name || '' });

    let query = supabase.from('transactions').select('*').eq('user_id', userId).order('created_at', { ascending: false });
    if (dateFilter === 'custom') {
      const fromDate = new Date(customFrom); fromDate.setHours(0, 0, 0, 0);
      ...
      switch (dateFilter) {
        ...
        case 'yesterday': since = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1); break;
        case 'last7': since = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000); break;
        case 'last30': since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); break;
      ...
      <header className="border-b border-[#00ff00]/30 pb-3 mb-4 flex justify-between items-end">
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-black border border-gray-800 rounded p-1 text-[10px]">
            <button onClick={() => setLang(lang === 'EN' ? 'JA' : 'EN')} className="px-2 py-1 hover:text-white transition-colors border-r border-gray-800">
              {lang === 'EN' ? '🇺🇸 EN' : '🇯🇵 JA'}
            </button>
            <button onClick={() => setCurrency(currency === 'JPY' ? 'USD' : 'JPY')} className="px-2 py-1 hover:text-white transition-colors">
              {currency === 'JPY' ? '💴 JPY' : '💵 USD'}
            </button>
          </div>
          <button onClick={() => router.push('/')} className="text-xs border border-[#00ff00]/50 text-[#00ff00] px-4 py-1.5 hover:bg-[#00ff00]/10 transition-colors">
            {t.back}
          </button>
        </div>
      </header>
    ...
    return (
      ...
      <div className="bg-[#0a0a0a] border border-[#00ff00]/20 p-3 mb-4 rounded space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {[
            { value: 'today', label: t.today }, { value: 'yesterday', label: t.yesterday },
            { value: 'last7', label: t.last7 }, { value: 'last30', label: t.last30 },
            { value: 'all', label: t.all }, { value: 'custom', label: t.custom },
          ].map((opt) => (
            <button key={opt.value} onClick={() => { setDateFilter(opt.value); setRangeError(''); }}
              className={`px-3 py-1 text-[10px] font-bold transition-colors ${dateFilter === opt.value ? 'bg-[#00ff00]/20 text-[#00ff00] border border-[#00ff00]' : 'text-gray-500 hover:text-white border border-gray-700'}`}>
              {opt.label}
            </button>
          ))}
          <span className="text-[10px] text-gray-500 ml-auto">{transactions.length} {t.filterSummary}</span>
        </div>
        {dateFilter === 'custom' && (
          <div className="pt-2 border-t border-gray-800 space-y-2">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <input type="date" value={customFrom} max={customTo} onChange={(e) => handleFromChange(e.target.value)}
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="date" value={customTo} min={customFrom} max={today} onChange={(e) => handleToChange(e.target.value)}
                  className="bg-black border border-gray-700 text-[#00ff00] text-xs p-1.5 rounded outline-none focus:border-[#00ff00] [color-scheme:dark]" />
              </div>
            </div>
            <p className="text-[10px] text-gray-500">📅 {customFrom} 〜 {customTo}{customFrom === customTo ? <span className="text-cyan-400 ml-1">（{new Date(customFrom).toLocaleDateString('ja-JP', { month: 'long', day: 'numeric', timeZone: 'Asia/Tokyo' })}のみ）</span> : ''}</p>
              ...
              {transactions.map((tx) => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-[#00ff00]/5 transition-colors">
                    ...
                    <td className="p-3"><span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${getTypeBadge(tx.type)}`}>{tx.type}</span></td>
                    ...
                    <td className="p-3 font-mono text-[9px] text-gray-500 max-w-[140px] truncate" title={tx.tx_hash || '(auto-generated)'}>
                      {hashDisplay}
                    </td>
                  </tr>
                );
